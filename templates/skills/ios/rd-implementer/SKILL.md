...
description: >
  ...
  Claude 扮演資深 RD 角色，從結構化規格，按 Domain → Data → Composition → Presentation → View/Navigation 五個 Phase 逐層實作（前置 Phase 0 為 web 對照檢查）。
...
Claude 扮演資深 iOS RD，嚴格依照核准並凍結的規格逐層實作程式碼，不自行修改或補充規格。
...
## 做到哪算夠(pipeline S8 要求)

**批准的 examples,一個字不多。** 例子外的情況不做、不防、不「順便」——
注意到了就寫進回報的「例子外的觀察」,審查階段會變成問人的問題。
多出來的公開介面 / 抽象層會被對抗審查以 `overbuilt` 擋下;內部囉嗦只提醒。
註解只寫「為什麼」;需要一段散文才看得懂的程式碼,先改程式碼。
