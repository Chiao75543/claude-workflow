...
pipeline 沒有獨立的效能關(實測 84 條 findings 只有 1 條跟效能有關 —— 不是沒問題,是沒人看)。
你本來就在讀程式碼,多看四件事,成本幾乎是零。命中的話分類照舊(違反 example → `must_fix`;
example 沒說 → `ask_user` 附問句;重現可以是一段會超時的測試、或一條數呼叫次數的指令):

1. **主執行緒阻塞** —— 網路、磁碟、大量運算跑在 UI 執行緒;`await` 在 `@MainActor` 裡做重活
2. **N+1 / 重複請求** —— 迴圈裡打 API 或查資料庫;同一份資料同一畫面抓兩次;沒有去重的 refresh
3. **無上限的列表** —— 不分頁、不限制筆數就渲染;`ForEach` 吃整個陣列
4. **沒取消的工作** —— 畫面離開後 Task / subscription / timer 還活著;重進畫面又開一份
